...
describe('track-drop-preview-store', () => {
  ...
  it('reuses unchanged per-track preview arrays across updates', () => {
    ...
    const firstTrackAPreviews =
      useTrackDropPreviewStore.getState().ghostPreviewsByTrackId['track-a']
    ...
    expect(useTrackDropPreviewStore.getState().ghostPreviewsByTrackId['track-a']).toBe(
      firstTrackAPreviews,
    )
  })
})
