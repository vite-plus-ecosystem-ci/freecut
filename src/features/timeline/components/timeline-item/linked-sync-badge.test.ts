...
function createParams(
  overrides: Partial<Parameters<typeof shouldSuppressLinkedSyncBadge>[0]> = {},
) {
  return {
...
describe('shouldSuppressLinkedSyncBadge', () => {
  it('does not suppress when linked selection is off', () => {
    expect(
      shouldSuppressLinkedSyncBadge(
        createParams({ linkedSelectionEnabled: false, isTrimming: true }),
      ),
    ).toBe(false)
  })
  ...
  it('does not suppress during move drags', () => {
    ...
    expect(
      shouldSuppressLinkedSyncBadge(
        createParams({ isDragging: true, linkedEditPreviewActive: true }),
      ),
    ).toBe(false)
  })

  it('suppresses for all linked tool preview paths', () => {
    expect(shouldSuppressLinkedSyncBadge(createParams({ linkedEditPreviewActive: true }))).toBe(
      true,
    )
